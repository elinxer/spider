...
//$url = 'https://www.zhihu.com/topic/19559424/top-answers?page=1'; // 文章列表页
...
foreach ($topic_list as $topic_key=>$topic)
{
    ...
    for ($i=1; $i <= 100; $i++)
    {
        ...
        if (!empty($list_urls))
        {
            foreach ($list_urls as $url_key => $url)
            {
                $tmp  = explode('/', $url);
                $code = isset($tmp[2])?$tmp[2]:'';
                $insert[] = array(
                    'question_title' => $list_titles[$url_key],
                    'question_url' => $url,
                    'question_code' => $code,
        ...
        db::insert_batch('zh_question_list', $insert);
        sleep(1);
    }
