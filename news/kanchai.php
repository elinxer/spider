...
{
    if (preg_match("#http://www.ikanchai.com/\d{8}/\d+.shtml#iUs", $item) ==false)
    {
...
foreach ($arr as $link)
{
    ...
    $article_wrap = selector::select($content_html, "//div[contains(@class, 'content')]");
    if (is_array($article_wrap)) {
        $article_wrap = current($article_wrap);
    }
    $title = selector::select($article_wrap, "//h2");
    if (is_array($title)) {
        $title = current($title);
    }
    $title = trim($title);
    $title = preg_replace("#<i class=\"tag\">.*?</i>#is",'', $title);
    $released_at = selector::select($article_wrap, "//div[@class='hl_c_twid']");
    ...
    $content = selector::select($article_wrap, "//div[contains(@class, 'hl_body')]");
    ...
    $tags = selector::select($article_wrap, "//div[@class='hl_c_tagl']/ul/li/a");
    if (!is_array($tags)) {
        $tags = array($tags);
    }
    $tags[] = '砍柴网';
    ...
    { //添加作者
        $author_id = db::insert('authors',
            [
                ...
                'author_desc'=> '',
                'author_avater' => 'http://img.ikanchai.com/skin/images/logo.jpg',
                'created_at' => date('Y-m-d H:i:s'),
