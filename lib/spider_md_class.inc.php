...
/**
 * @author Elinx
 * Class spider_md_class
 */


/*

require 'spider_md_class.inc.php';
header("Content-type: text/html; charset=utf-8");
$obj             = new spider_md_class();
...
class spider_md_class {
    ...
    /**
     * html转 markdown @Elinx 完善自 Zjmainstay
     * 说明：换行仅针对linux有效(\n)，
     * 使用之前请先进行数据清洗，保证待转换html没有过多特殊标签和特殊符号
     *
     *   //php 不同系统的换行
     *   //不同系统之间换行的实现是不一样的
     *   //linux 与unix中用 \n
    ...
    public function parseHtml($html) {
        ...
        // 清洗图片数据
        $html = preg_replace_callback("/<img.*?src=\"(.*?)\".*?>/is",
            function ($matches) {
                $img_url = $matches[1];
                if (!empty($img_url)) {
                    return "![]({$img_url})";
